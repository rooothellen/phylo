...
## Phylogenetic relationships among taxa can be described by indicating each monophyletic clade. If we enumerate every monophyletic clade in a phylogeny, then we have all the information to draw the tree. One way to do this is to describe monophyletic clades using parentheses. See the example below:
...
## Go check wikipedia: https://en.wikipedia.org/wiki/Newick_format
...
plot.phylo( phy )
## How this phylogeny compares to the original tree in Baum et al. (2005) paper?
...
help( read.tree ) # Do you see other ways to use this function?
help( plot.phylo ) # See all the extra arguments we can use to adjust the plot?
